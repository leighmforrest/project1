...
class RegistrationForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired(), Length(
        min=5, max=25, message="Username must be between 5 and 25 characters")])
    handle = StringField('Handle', validators=[Length(max=25, message="Username cannot exceed 25 characters")])
